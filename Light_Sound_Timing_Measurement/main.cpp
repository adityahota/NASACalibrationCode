...
void initFILE() {
    FILE *fp = fopen("/local/data_light_sound.csv", "w");
    if (fp == NULL) {
        printf("File error\r\n");
    } else {        
        fprintf(fp, "Light to Sound\n");
        fclose(fp);
    }
}  
...
extern "C" void EINT3_IRQHandler(void) __irq {
    ...
    LPC_GPIO0->FIOCLR |= (1 << 1);                 // Stop voltage to p20 (solenoid)
    ...
    FILE *fp = fopen("/local/data_light_sound.csv", "a");
    if (fp == NULL) {
        printf("File error\r\n");
    } else {        
        fprintf(fp, "%d\n", soundSenseTime - lightSenseTime);
        fclose(fp);
    }
